...
public class Screen {
	...
	public boolean addShape(Shape shapeObject) {
		...
		try {
			...
			if (shapeObject.getOrigin().x < 0 || shapeObject.getOrigin().y < 0
					|| shapeObject.getOrigin().x > Point.XMAX
					|| shapeObject.getOrigin().y > Point.YMAX) {
	...
	public List<Shape> sortByOriginDistance() {
		...
		List<Shape> sortedListByOriginDistance = new ArrayList<Shape>(
				shapesOnScreen);

		Collections.sort(sortedListByOriginDistance,
				ShapeSort.sortByOriginDistance);
